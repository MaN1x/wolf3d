...
/*   Updated: 2020/11/26 18:11:36 by mjoss            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_ray_map(t_wolf3d *wolf, t_player player, t_ray *ray)
{
	float	ray1_len;
	float	ray2_len;
	float	ray1_len_down;
	...
	ray4hit = ray->ray_angle > M_PI ? ray->ray_angle - M_PI :
			ray->ray_angle + M_PI;
	ray1_len_down = draw_h_down(wolf, player, ray4hit);
	ray2_len_down = draw_v_down(wolf, player, ray4hit);
	ray1_len = draw_horisontal(wolf, player, ray);
	ray2_len = draw_vertical(wolf, player, ray);
	if (ray1_len < 15 || ray2_len < 15)
		wolf->is_hit = 1;
	if (ray1_len_down < 15 || ray2_len_down < 15)
		wolf->is_hit_down = 1;
	if (ray1_len < ray2_len)
		ray->ray_len = draw_horisontal(wolf, player, ray);
	else
		ray->ray_len = draw_vertical(wolf, player, ray);
}
...
void	draw_rays(t_wolf3d *wolf, t_player player)
{
	...
	while (i < SCREEN_WIDTH)
	{
		...
		draw_ray_map(wolf, player, &ray[i]);
